...
import { Display, LoginResponse } from "@sakuli-dashboard/api";
...
export const LogDisplay: React.FC<LogDisplayProps> = (props: LogDisplayProps) => {
    ...
    useEffect(() => {
        getLogsOfAction(props.display.actionIdentifier!, props.loginInformation?.jwtToken)
            .then((receivedLogs) => setLogs(receivedLogs))
            .then(() => sleep(pollingInterval))
            .then(() => setLastPolling(Date.now() - pollingInterval));
    }, [lastPolling, props.display.actionIdentifier, props.loginInformation?.jwtToken]);
